...
extern void quarterround_asm(uint32 *a, uint32 *b, uint32 *c, uint32 *d);
extern void crypto_core_chacha20_asm(
  unsigned char *out,
...
int crypto_stream_chacha20(unsigned char *c,unsigned long long clen, const unsigned char *n, const unsigned char *k)
{
  ...
  while (clen >= 64) {
    crypto_core_chacha20_asm(c,in,kcopy,sigma);
  ...
  if (clen) {
    crypto_core_chacha20_asm(block,in,kcopy,sigma);
